...
export class AuthMiddleware implements IMiddleware {
    public use(
        ...
        if (!request.isAuthenticated()) {
            ...
            const error = new Unauthorized(
                AuthMiddlewareErrorMessages[
                    AuthMiddlewareErrorKeys.UNAUTHORIZED
                ]
            );
            error.body = createCustomErrorBody(
                AuthMiddlewareErrorKeys.UNAUTHORIZED,
                AuthMiddlewareErrorMessages
            );
            return next(error);
        ...
        const isRoleAuthenticated = _.some(userRoles, userRole =>
            _.includes(options.roles, userRole)
        );

        if (!isRoleAuthenticated) {
            const forbiddenError = new Forbidden(
                AuthMiddlewareErrorMessages[AuthMiddlewareErrorKeys.FORBIDDEN]
            );
            return next({
                ...forbiddenError,
                body: createCustomErrorBody(
                    AuthMiddlewareErrorKeys.FORBIDDEN,
                    AuthMiddlewareErrorMessages
                )
            });
        }
